...
but what is the type of an identifier? Clearly, we need type 
...
where $\tau_{a}$ is the type of the expression \textit{a}
...
where the two type annotations are now required: the one immediately after
...
(notice that we have dropped the overly naive type \texttt{TFunction}
...
is (TArrow TInt(TArrow TInt TInt)), while the inner function has type
(TArrow TInt TInt).
