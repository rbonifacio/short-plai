...
but you have used them in every programming language
you have employed so far: they are called \emph{variables}.
...
incidentally, notice that the act of reducing an expression to
...
Fortunately, Haskell has a primitive type called String, which will
serve this role admirably. Nevertheless, it
...
while the rest are calculations, as defined for \ae.
For now, we are going to pin down substitution.
...
\begin{mydef}[Substitution, take 3]{def:substitution2}
Given an expression like $Let\ x =\ exp_1\ in\ exp_2$,
the components of the \texttt{Let} expression are the
identifier \texttt{$x$}, the named expression \texttt{$exp_1$},
and expression body \texttt{$exp_2$}. To substitute
...
\begin{mydef}[Substitution, take 4]{def:substitution2}
Given an expression like $Let\ x =\ exp_1\ in\ exp_2$,
the components of the \texttt{Let} expression are the
identifier \texttt{$x$}, the named expression \texttt{$exp_1$},
and expression body \texttt{$exp_2$}. To substitute
...
substituting a \texttt{Ref i} expression, we have
to deal with two new situations. The first,
when trying to substitute the identifier \texttt{x}
...
named expression, then substitute the identifier by its value
in the body of the \texttt{Let}
expression and, lastly, we evaluate the body.
...
calculator encounters that identifier's binding instance. Consequently,
